...
void GroeiTH::Compress(Config *config, const string tagIn) {
...
void GroeiTH::DoCompress(Config *config, Database *db, ItemSetCollection *isc, const string &tag, const string &timetag,
    ...
    ECHO(1, printf(" * Algorithm:\t\t%s", algoName.c_str()));
    {
        ...
        GroeiAlgo* algo_ng = static_cast<GroeiAlgo*>(algo);
        if (algo_ng) {
