...
namespace tlmnr {
	...
	{
	public:
		Output(TileMiner *cr) { mCroupier = cr; mHasBinSizes = false; mFinished = false; }
		virtual ~Output() {
			mCroupier = NULL;
		}
	...
	{
	public:
		MemoryOut(Database *db, TileMiner *cr);
		virtual ~MemoryOut();
	...
	public:
		...
		FSout(string filename, TileMiner *cr);
		virtual ~FSout();
